...
def getAllData(url, tempArray=[]):
    ...
    try:
        ...
        pathurl = soup.select('ul.uk-pagination.uk-align-right.top-paging > li:nth-child(8) a')[0].get('href')
...
html_doc = requests.get('https://sinta.kemdikbud.go.id/affiliations/detail?id=2042&view=overview')
soup = BeautifulSoup(html_doc.text, 'html.parser')
...
with open("Scrapping Sinta Ubhara.json", "a") as outfile:
